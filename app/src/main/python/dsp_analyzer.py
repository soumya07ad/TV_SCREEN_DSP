...
def analyze_audio(wav_path: str) -> Dict:
    ...
    try:
        ...
        if not samples:
            return {"error": "Empty audio file or unsupported format"}
...
def calculate_power_db(samples: List[float]) -> float:
...
def find_dominant_frequency(samples: List[float], sample_rate: int) -> float:
    """
    Find dominant frequency using autocorrelation.
...
def calculate_spectral_flatness(samples: List[float]) -> float:
    """
    Calculate spectral flatness (surface tension proxy).
    Pure Python approximation without FFT.
    """
    if len(samples) < 100:
        return 0.0
...
def classify_noise(frequency: float, power_db: float, surface_tension: float) -> Tuple[str, float]:
    """
